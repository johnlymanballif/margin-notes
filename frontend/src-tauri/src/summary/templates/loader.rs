...
/// - macOS: ~/Library/Application Support/Margin Notes/templates/
/// - Windows: %APPDATA%\Margin Notes\templates\
/// - Linux: ~/.config/Margin Notes/templates/
fn get_custom_templates_dir() -> Option<PathBuf> {
    let mut path = dirs::data_dir()?;
    path.push("Margin Notes");
    path.push("templates");
    Some(path)
}
