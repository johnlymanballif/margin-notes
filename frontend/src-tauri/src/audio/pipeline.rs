...
struct AudioMixerRingBuffer {
    ...
    mic_overflow_count: usize,
    system_overflow_count: usize,
...
impl AudioMixerRingBuffer {
    fn new(sample_rate: u32) -> Self {
        ...
        // Start with conservative size, will adapt based on overflow frequency
        let base_multiplier = 8; // 400ms base
        let max_buffer_size = window_size_samples * base_multiplier;

        info!("🔊 Ring buffer initialized: window={}ms ({} samples), max={}ms ({} samples)",
              window_ms, window_size_samples,
              window_ms * base_multiplier as f32, max_buffer_size);
        ...
        Self {
            ...
            system_overflow_count: 0,
            total_samples_processed: 0,
            last_health_check: std::time::Instant::now(),
        }
    }

    /// Dynamically adjust buffer size based on overflow frequency
    /// Returns true if buffer was resized
    fn adapt_buffer_size(&mut self, overflow_count: usize, total_samples: usize) -> bool {
        if total_samples == 0 {
            return false;
        }

        let overflow_rate = overflow_count as f32 / total_samples as f32;
        
        // If overflow rate > 1%, increase buffer size
        if overflow_rate > 0.01 && self.max_buffer_size < self.window_size_samples * 16 {
            let old_size = self.max_buffer_size;
            self.max_buffer_size = (self.max_buffer_size as f32 * 1.2) as usize;
            info!("📈 Buffer size increased due to overflow: {} -> {} samples (overflow rate: {:.2}%)",
                  old_size, self.max_buffer_size, overflow_rate * 100.0);
            return true;
        }
        
        // If overflow rate < 0.1%, decrease buffer size (but not below base)
        if overflow_rate < 0.001 && self.max_buffer_size > self.window_size_samples * 6 {
            let old_size = self.max_buffer_size;
            self.max_buffer_size = (self.max_buffer_size as f32 * 0.9) as usize;
            info!("📉 Buffer size decreased due to low overflow: {} -> {} samples (overflow rate: {:.2}%)",
                  old_size, self.max_buffer_size, overflow_rate * 100.0);
            return true;
        }
    ...
    fn add_samples(&mut self, device_type: DeviceType, samples: Vec<f32>) {
        ...
        // Track overflow before dropping samples
        let mic_overflow = if self.mic_buffer.len() > self.max_buffer_size {
            self.mic_buffer.len() - self.max_buffer_size
        } else {
            0
        };
        
        let system_overflow = if self.system_buffer.len() > self.max_buffer_size {
        ...
        if mic_overflow > 0 {
            self.mic_overflow_count += mic_overflow;
            warn!("⚠️ Microphone buffer overflow: {} > {} samples, dropping oldest {} samples",
                  self.mic_buffer.len(), self.max_buffer_size, mic_overflow);
        }
        if system_overflow > 0 {
            self.system_overflow_count += system_overflow;
            error!("🔴 SYSTEM AUDIO BUFFER OVERFLOW: {} > {} samples, dropping {} samples - THIS CAUSES DISTORTION!",
                  self.system_buffer.len(), self.max_buffer_size, system_overflow);
        }

        // Safety: prevent buffer overflow (keep only last max_buffer_size samples)
        ...
        // Adaptive buffer sizing: check health every 5 seconds
        if self.last_health_check.elapsed().as_secs() >= 5 {
            let total_overflow = self.mic_overflow_count + self.system_overflow_count;
            if self.adapt_buffer_size(total_overflow, self.total_samples_processed) {
                // Reset counters after adaptation
                self.mic_overflow_count = 0;
                self.system_overflow_count = 0;
