...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\margin-notes-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("margin-notes-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("margin-notes-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/margin-notes-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("margin-notes-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/margin-notes-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("margin-notes-recordings")
    }
