...
impl MeetingsRepository {
    pub async fn get_meetings(pool: &SqlitePool) -> Result<Vec<MeetingModel>, sqlx::Error> {
        let meetings =
            sqlx::query_as::<_, MeetingModel>("SELECT id, title, created_at, updated_at, folder_path, folder_id FROM meetings ORDER BY created_at DESC")
                .fetch_all(pool)
                .await?;
        Ok(meetings)
    }
    ...
    pub async fn get_meeting(
        ...
        if let Some(meeting) = meeting {
            // Get all transcripts for this meeting (for now, keep full fetch for backward compatibility)
            // Pagination can be added via a separate method if needed
            let transcripts =
                sqlx::query_as::<_, Transcript>("SELECT * FROM transcripts WHERE meeting_id = ? ORDER BY audio_start_time ASC, sequence_id ASC")
                    .bind(meeting_id)
    ...
    pub async fn get_meeting_transcripts_paginated(
        ...
        let transcripts = sqlx::query_as::<_, Transcript>(
            "SELECT * FROM transcripts WHERE meeting_id = ? ORDER BY audio_start_time ASC, sequence_id ASC LIMIT ? OFFSET ?"
        )
        .bind(meeting_id)
        .bind(limit)
        .bind(offset)
