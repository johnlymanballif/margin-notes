...
const Sidebar: React.FC = () => {
  ...
  const handleAdvancedSearch = useCallback(async (filters: SearchFilters) => {
    ...
    // If search query is empty and no filters, just return to normal view
    if (!filters.query.trim() && !filters.dateFrom && !filters.dateTo && !filters.speakerId && !filters.companyId) {
      return;
    }
    
    // Search through transcripts (for now, use simple search - can be enhanced with filters)
    if (filters.query.trim()) {
      await searchTranscripts(filters.query);
    }
  ...
  // State for meeting tags
  const [meetingTags, setMeetingTags] = useState<Map<string, any[]>>(new Map());

  // Load tags for meetings
  useEffect(() => {
    const loadMeetingTags = async () => {
      const tagsMap = new Map<string, any[]>();
      for (const meeting of meetings) {
        try {
          const tags = await invoke<any[]>('get_meeting_tags', { meetingId: meeting.id });
          if (tags && tags.length > 0) {
            tagsMap.set(meeting.id, tags);
          }
        } catch (error) {
          // Ignore errors for individual meetings
        }
      }
      setMeetingTags(tagsMap);
    };
  ...
  const renderItem = (item: SidebarItem, depth = 0) => {
    ...
    const isFolderItem = item.type === 'folder' && item.id !== 'meetings';
    ...
    const tags = isMeetingItem ? meetingTags.get(item.id) || [] : [];
        ...
        >
          {item.type === 'folder' ? (
            <>
              ...
              {isFolderItem && item.children && item.children.length > 0 && (
                <span className="ml-2 text-xs text-gray-400">({item.children.length})</span>
              )}
