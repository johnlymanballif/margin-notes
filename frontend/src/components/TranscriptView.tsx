...
const TranscriptItem = React.memo(({
  ...
  meetingId?: string;
  speakers: Speaker[];
  streamingTranscript: { id: string; visibleText: string; fullText: string } | null;
  showConfidence: boolean;
  handleSpeakerChange: (transcriptId: string, speakerId: string | null) => void;
}) => {
  ...
  const filteredText = useMemo(() => cleanStopWords(textToShow), [textToShow]);
  ...
  const sizerText = useMemo(() => 
    ...
    [isStreaming, streamingTranscript?.fullText, transcript.text, originalWasEmpty]
  );
  ...
  return (
    <motion.div
      ...
      className="mb-3 group hover:bg-gray-50 rounded-lg p-2 transition-colors"
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          {!isRecording && meetingId && (
            <div className="mb-1 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <Select
                value={transcript.speaker_id || 'none'}
                onValueChange={(value) => handleSpeakerChange(transcript.id, value === 'none' ? null : value)}
              >
                  ...
                  <SelectValue>
                    {transcript.speaker_name ? (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="truncate">{transcript.speaker_name}</span>
                      </div>
                    ) : (
                      <span className="text-gray-400">Assign speaker</span>
                    )}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
          ...
          {/* Speaker Badge (when assigned) */}
          {transcript.speaker_name && (
            <div className="mb-1 flex items-center gap-1">
              <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-xs">
                <User className="h-3 w-3" />
                <span className="font-medium">{transcript.speaker_name}</span>
                {transcript.speaker_id && speakers.find(s => s.id === transcript.speaker_id)?.company_name && (
                  <>
                    <span className="mx-1">•</span>
                    <Building2 className="h-3 w-3" />
                    <span>{speakers.find(s => s.id === transcript.speaker_id)?.company_name}</span>
                  </>
                )}
              </div>
            </div>
...
export const TranscriptView: React.FC<TranscriptViewProps> = ({ 
  ...
  const { speakers, isLoading: loadingSpeakers } = useSpeakers();
  ...
  // Speakers are now loaded from context, no need for local loading

  const handleSpeakerChange = useCallback(async (transcriptId: string, speakerId: string | null) => {
    try {
      await invoke('update_transcript_speaker', {
        transcriptId,
        speakerId,
      });
      ...
      if (onTranscriptUpdate) {
        ...
        onTranscriptUpdate(transcriptId, {
          speaker_id: speakerId || undefined,
          speaker_name: speaker?.name || undefined,
        });
      }
    } catch (error) {
      console.error('Failed to update speaker:', error);
    }
  }, [speakers, onTranscriptUpdate]);
  ...
  return (
    <div className="px-4 py-2">
      ...
      {/* Virtual scrolling for performance with large transcript lists (100+ items) */}
        ...
        <Virtuoso
          ...
          itemContent={(index, transcript) => {
            return <TranscriptItem
              key={transcript.id || `transcript-${index}`}
              transcript={transcript}
              index={index}
          ...
          followOutput="smooth"
          initialTopMostItemIndex={transcripts.length > 0 ? transcripts.length - 1 : 0}
      ...
      {transcripts.length === 0 && (
        ...
        >
          {isRecording ? (
            ...
            <>
              <p className="text-lg font-semibold">Welcome to Margin Notes!</p>
