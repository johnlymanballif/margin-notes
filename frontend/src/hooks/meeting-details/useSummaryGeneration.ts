...
export function useSummaryGeneration({
  ...
  const processSummary = useCallback(async ({
    ...
    try {
      ...
      // Load master prompt and combine with custom prompt
      let masterPrompt = '';
      try {
        masterPrompt = await invokeTauri<string>('get_master_prompt');
      } catch (error) {
        console.warn('Failed to load master prompt:', error);
      }

      // Combine master prompt with custom prompt
      const combinedPrompt = masterPrompt.trim()
        ? `${masterPrompt.trim()}\n\n${customPrompt.trim()}`.trim()
        : customPrompt.trim();

      // Process transcript and get process_id
      // Optimized chunking: reduced from 40000 to 25000 for faster processing
      // and better quality (smaller chunks = more context per chunk)
      const result = await invokeTauri('api_process_transcript', {
        text: transcriptText,
        model: modelConfig.provider,
        modelName: modelConfig.model,
        meetingId: meeting.id,
        chunkSize: 25000, // Reduced from 40000 for better performance
        overlap: 1000,
